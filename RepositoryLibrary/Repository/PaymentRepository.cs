...
public class PaymentRepository : IPaymentRepository
{
    ...
    public async Task<List<UserPayment>> GetPaymentsByUserId(string userId) //0references
    {
    ...
    public async Task<List<UserPayment>> GetAllPayments() //0 references
    {
    ...
    public async Task<List<UserPayment>> GetPaymentsByMonth(DateOnly date) //0references
    {
    ...
    public async Task<UserPayment> CreatePayment(UserPayment payment) // 0 references
    {
    ...
    public async Task<UserPayment> DeletePayment(UserPayment payment) //0 references
    {
    ...
    public async Task<UserPayment> EditPayment(UserPayment payment) // 0 references
    {
        var existing = await _dbContext.UserPayments
      .FirstOrDefaultAsync(up => up.UserId == payment.UserId
                              && up.BuyDate == payment.BuyDate);
    ...
    public async Task<(bool weekly, int? amount)> IsWeekly(string userId) //1 reference
    {
        ...
        if (payment.PackageBought.Weekly) 
            return (true, payment.PackageBought.ClassesIncluded);
    ...
    public async Task useClass(string userId) // 0 references
    {
